  ...
  parseValue(value: any) {
    ...
    if (typeof value === 'string') {
      ...
      if (isNaN(date.getTime())) {
        throw new Error('GraphQL Date Scalar parser received invalid date string');
      }
      return date;
    }
    throw new Error('GraphQL Date Scalar parser expected a `number` or valid date string');
  },
  parseLiteral(ast) {
    ...
    if (ast.kind === Kind.STRING) {
      ...
      if (isNaN(date.getTime())) {
        throw new Error('GraphQL Date Scalar parseLiteral received invalid date string');
      }
      return date;
    }
    return null;
...
export const taskResolvers = {
  ...
  Query: {
    getTasks: async (
      _: any,
      { filter, limit, offset, sortBy, sortOrder }: any,
      { userId }: Context
    ) => {
      try {
    ...
    getTask: async (_: any, { id }: any, { userId }: Context) => {
      try {
    ...
    getTasksByDate: async (_: any, { date }: any, { userId }: Context) => {
      try {
    ...
    searchTasks: async (
      ...
      { userId, elasticsearchService }: Context
    ) => {
      try {
    ...
    getTaskStats: async (_: any, __: any, { userId }: Context) => {
      try {
        ...
        const [
          ...
          Task.countDocuments({ userId, pinned: true }),
          Task.countDocuments({ 
            userId, 
  ...
  Mutation: {
    createTask: async (_: any, { input }: any, { userId, elasticsearchService }: Context) => {
      try {
        const task = new Task({
          ...input,
          userId,
    ...
    updateTask: async (_: any, { id, input }: any, { userId, elasticsearchService }: Context) => {
      try {
    ...
    deleteTask: async (_: any, { id }: any, { userId, elasticsearchService }: Context) => {
      try {
    ...
    toggleTaskComplete: async (_: any, { id }: any, { userId, elasticsearchService }: Context) => {
      try {
    ...
    toggleTaskPin: async (_: any, { id }: any, { userId, elasticsearchService }: Context) => {
      try {
        const task = await Task.findOne({ _id: id, userId });
    ...
    bulkDeleteTasks: async (_: any, { ids }: any, { userId, elasticsearchService }: Context) => {
      try {
        ...
        try {
          await Promise.all(
            ids.map((id: string) => elasticsearchService.deleteTask(id))
          );
        } catch (esError) {
    ...
    bulkUpdateTasks: async (_: any, { ids, input }: any, { userId, elasticsearchService }: Context) => {
      try {
        const updateData = { ...input };
        if (input.priority) {
          updateData.priority = input.priority.toLowerCase();
        ...
        const updatedTasks = await Task.find({ _id: { $in: ids }, userId });

        // Update in Elasticsearch
        try {
          await Promise.all(
            updatedTasks.map(task => elasticsearchService.updateTask(task))
          );
        } catch (esError) {
